...
export async function onRequest(context) {
  ...
  try {
    ...
    if (path === '/api/check-reservation' && method === 'GET') {
      ...
      const result = await env.DB.prepare('SELECT COUNT(*) as count FROM reservations WHERE patient_nin = ?').bind(nin).first();
    ...
    if (path === '/api/reservations' && method === 'POST') {
      const { slot_id, nin, first_name, last_name, dob, letter_base64, parent_nin, parent_first_name, parent_last_name, parent_dob } = await request.json();
      
      // Check if NIN already has a reservation
      const existing = await env.DB.prepare('SELECT id FROM reservations WHERE patient_nin = ?').bind(nin).first();
      if (existing) return new Response(JSON.stringify({ error: 'لديك حجز مسبق بالفعل' }), { status: 409, headers });
